define(function(require) {
  ...
  Table.prototype._draw = function(tableAndTypes) {
    ...
    function createDimensionName(type) {
      return type.name + " [" + type.type + ((type.nullable) ? "*" : "") + "]";
    }

    function isNumeric(type) {
      return type == "double" || type == "integer" || type == "float" || type == "byte" || type == "short" ||
        type == "long" || type == "decimal" || type == "date" || type == "timestamp";
    }

    var types = {};
    // TODO types are a string with JSON inside so I gotta parse it
    tableAndTypes.schema.map(function(type) {
      var dimensionName = createDimensionName(type);
      // TODO how to identify null values? I have to go through all values and check if one column is comletely null
      types[dimensionName + " "] = (isNumeric(type)) ? "number" : "string";
    });
    var table = tableAndTypes.content.map(function(row) {
      var rowObject = {};
      row.map(function(cell, columnIdx) {
        rowObject[createDimensionName(tableAndTypes.schema[columnIdx])] = cell;
      });
      return rowObject;
    });
    ...
    if (shouldDrawParcoords) {
      ...
      if (isNumeric(types[Object.keys(types)[0]]) && singleColumn.length > 1) {
